...
@click.option(
    "--format", "fmt",
    type=click.Choice(["markdown", "appendix"]),
    default="markdown",
    help="Output format. 'markdown' emits a plain definition list; "
         "'appendix' wraps it as a numbered appendix section.",
)
@click.option(
    "--output", "output_path",
    default=None,
...
def paper_glossary(fmt, output_path):
    """Generate a reader-facing glossary from the shared ontology.

    Reads controlled vocabulary terms from shared_ontology vocabulary_rules.yaml
    files, filters to terms that actually appear in paper section files, extracts
    definitions from the vocabulary markdown files, and emits a formatted glossary
    suitable for inclusion as an appendix.

    This is a build artifact — do not commit it as a hand-edited file. Regenerate
    whenever paper content or ontology changes.
    """
    project_dir = Path.cwd()
    config = load_project_config(project_dir)

    vocab_paths = get_shared_ontology_sources(config)
    if not vocab_paths:
        click.echo(
            "Error: shared_ontology not configured in seldon.yaml. "
            "Add shared_ontology.source and shared_ontology.vocabularies to enable this command.",
            err=True,
        )
        raise SystemExit(1)
    ...
    if not rule_paths:
        click.echo(
            "Error: no vocabulary_rules.yaml found alongside vocabulary files. "
            "At least one vocabulary_rules.yaml is required.",
            err=True,
        )
        raise SystemExit(1)

    # Load term names (from rules) and definitions (from vocabulary markdown)
    terms, banned, exemptions = load_vocabulary_rules(rule_paths)
    definitions = get_term_definitions(vocab_paths)

    # Scan sections to find which terms are actually used
    sections_dir = get_sections_dir(config, project_dir)
    section_paths = sorted(sections_dir.glob("*.md")) if sections_dir.exists() else []

    used_terms: set[str] = set()
    for sf in section_paths:
        found, _ = scan_section(sf, terms, banned, exemptions)
        used_terms.update(found.keys())

    if not used_terms:
        click.echo(
            "Warning: no controlled vocabulary terms found in section files. "
            "Glossary will be empty.",
            err=True,
        )

    # Build glossary output
    sorted_terms = sorted(used_terms, key=str.lower)
    lines: list[str] = []
    ...
    if fmt == "appendix":
        lines += [
            ...
            "",
            "Definitions for controlled vocabulary terms used in this paper. "
            "Sourced from the Seldon shared ontology (AD-017).",
            "",
        ]
    else:
        lines += [
            "# Glossary",
            "",
            "Controlled vocabulary terms used in this paper.",
            "",
        ]
    ...
    if not sorted_terms:
        lines.append("*No controlled vocabulary terms found in section files.*")
        lines.append("")

    lines.append(
        f"*Generated by `seldon paper glossary`. "
        f"Source: {', '.join(str(p) for p in vocab_paths)}.*"
    )
